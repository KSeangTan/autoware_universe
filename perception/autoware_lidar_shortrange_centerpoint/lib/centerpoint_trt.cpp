...
#include "autoware/lidar_shortrange_centerpoint/centerpoint_trt.hpp"

#include "autoware/lidar_shortrange_centerpoint/centerpoint_config.hpp"
#include "autoware/lidar_shortrange_centerpoint/network/scatter_kernel.hpp"
#include "autoware/lidar_shortrange_centerpoint/preprocess/preprocess_kernel.hpp"
  ...
  std::vector<Box3D> & det_boxes3d, bool & is_num_pillars_within_range)
{
  ...
  if (!preprocess(input_pointcloud_msg, tf_buffer)) {
    RCLCPP_WARN(rclcpp::get_logger("lidar_shortrange_centerpoint"), "Fail to preprocess and skip to detect.");
    return false;
  }
  ...
  if (num_pillars >= config_.max_voxel_size_) {
    ...
    RCLCPP_WARN_THROTTLE(
      rclcpp::get_logger("lidar_shortrange_centerpoint"), clock, 1000,
...
void CenterPointTRT::postProcess(std::vector<Box3D> & det_boxes3d)
{
  ...
  if (det_boxes3d.size() == 0) {
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("lidar_shortrange_centerpoint"), "No detected boxes.");
  }
}
