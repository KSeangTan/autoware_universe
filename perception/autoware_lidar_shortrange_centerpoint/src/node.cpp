...
#include "autoware/lidar_shortrange_centerpoint/node.hpp"
...
: Node("lidar_shortrange_center_point", node_options), tf_buffer_(this->get_clock())
{
  ...
  if (point_cloud_range.size() != 6) {
    RCLCPP_WARN_STREAM(
      rclcpp::get_logger("lidar_shortrange_centerpoint"),
  ...
  // initialize debug tool
  {
    ...
    debug_publisher_ptr_ = std::make_unique<DebugPublisher>(this, "lidar_shortrange_centerpoint");
  ...
  if (stop_watch_ptr_) {
    ...
    diagnostic_processing_time_updater_.add(
      "processing_time_status", this, &LidarShortRangeCenterPointNode::diagnoseProcessingTime);
    // msec -> sec
...
void LidarShortRangeCenterPointNode::diagnoseProcessingTime(
