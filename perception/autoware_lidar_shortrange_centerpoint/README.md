# autoware_lidar_shortrange_centerpoint

## Purpose

autoware_lidar_shortrange_centerpoint is a package for detecting dynamic 3D objects.
...
The `autoware_lidar_shortrange_centerpoint` node has `build_only` option to build the TensorRT engine file from the ONNX file.
...
The autoware_lidar_shortrange_centerpoint implementation requires two ONNX models as input the voxel encoder and the backbone-neck-head of the CenterPoint model, other aspects of the network,
...
Create a new config file named **centerpoint_custom.param.yaml** under the config file directory of the autoware_lidar_shortrange_centerpoint node. Sets the parameters of the config file like
