...
namespace autoware::lidar_centerpoint
{
	...
	const float * transform_array, float * output_points)
	{
		...
		generateSweepPoints_kernel<<<blocks, threads, 0, stream_>>>(
			input_points, points_size, time_lag, transform_array, config.point_feature_size,
			output_points);
	...
	const float * points, std::size_t points_size, unsigned int * mask, float * voxels)
	{
		...
		generateVoxels_random_kernel<<<blocks, threads, 0, stream_>>>(
			points, points_size, config_.range_min_x_, config_.range_max_x_,
			...
			config_.grid_size_x_, mask, voxels);
		cudaError_t err = cudaGetLastError();
		return err;
	}
	...
	const float * voxel_features, const float * voxel_num_points, const int * coords, const unsigned int * num_voxels, float * features)
	{
		dim3 blocks(divup(config_.max_voxel_size_, WARPS_PER_BLOCK));
		dim3 threads(WARPS_PER_BLOCK * MAX_POINT_IN_VOXEL_SIZE);
		if (config_.encoder_in_feature_size_ == 9) {
			generateFeatures_kernel<9><<<blocks, threads, 0, stream_>>>(
			voxel_features, voxel_num_points, coords, num_voxels, config_.voxel_size_x_, config_.voxel_size_y_, config_.voxel_size_z_,
			config_.range_min_x_, config_.range_min_y_, config_.range_min_z_, features);
		} else if (config_.encoder_in_feature_size_ == 10) {
			generateFeatures_kernel<10><<<blocks, threads, 0, stream_>>>(
			voxel_features, voxel_num_points, coords, num_voxels, config_.voxel_size_x_, config_.voxel_size_y_, config_.voxel_size_z_,
			config_.range_min_x_, config_.range_min_y_, config_.range_min_z_, features);
		} else {
